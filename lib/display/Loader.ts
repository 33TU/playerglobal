...
export class Loader extends DisplayObjectContainer implements ILoader {
	...
	private _onAssetComplete(event: AssetEvent) {
		...
		if (asset.isAsset(AwayTextField)) {
			...
			this._contentLoaderInfo.applicationDomain.addDefinition(asset.name, <SceneImage2D> asset);
			...
			if (asset.name == 'Scene 1' || (<any>asset).isAVMScene) {
				...
				FrameScriptManager.addPendingAS3Constructor(this._content.adaptee);
	 ...
	 * * Event listeners for `enterFrame`, `frameConstructed`, `exitFrame`,
